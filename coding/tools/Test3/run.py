...
Vincomplete = rd.random_remove_data(Vl,percent=5);

Vcomplete = [];

for i in range(len(Vincomplete)): # 0,1
    Vcomplete.append(rd.remove_nan(Vincomplete[i]));
...
Wl,Hl,Zl,Gl,w0,S,F= al.implement(Vincomplete,Vcomplete, label, alpha[0], beta[0], gamma[0], lambdas[0]);
